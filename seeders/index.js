...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('I1aQGq79_lObfNX') ) {
      let user = new User({
        'password':'I1aQGq79_lObfNX',
        'username':'Olen11',
    ...
    if (!admin || !admin.isPasswordMatch('_jA6A6urySJ_eLm') ) {
      let admin = new User({
        'password':'_jA6A6urySJ_eLm',
        'username':'Zion82',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('I1aQGq79_lObfNX') && userRole){
    ...
    if (admin && admin.isPasswordMatch('_jA6A6urySJ_eLm') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
